export default {
  async fetch(request, env, ctx) {
    ...
    try {
      const data = await buildLolSummary(env);
...
async function buildLolSummary(env) {
  const RIOT_API_KEY = env.RIOT_API_KEY;

  if (!RIOT_API_KEY) {
    throw new Error("Missing RIOT_API_KEY secret");
  }
  ...
  const matchIds = await riotFetch(
    `https://${PROFILE.routingRegion}.api.riotgames.com/lol/match/v5/matches/by-puuid/${account.puuid}/ids?start=0&count=${PROFILE.matchCount}`
  );

  const matches = await Promise.all(
    matchIds.map((matchId) =>
      riotFetch(
        `https://${PROFILE.routingRegion}.api.riotgames.com/lol/match/v5/matches/${matchId}`
      )
    )
  );
  ...
  const rawTop = [...championMap.values()]
    ...
    .slice(0, 5)
    .map((champ) => {
      const kda = safeDivide(
        champ.kills + champ.assists,
        Math.max(1, champ.deaths)
      );
  ...
  function statRange(arr, fallbackMin, fallbackMax) {
    return {
  ...
  const dmgRange = statRange(
    rawTop.map((x) => x.dmgRaw),
    0,
    1000
  );
  const wardRange = statRange(
    rawTop.map((x) => x.wardsRaw),
    0,
    2
  );
  const csRange = statRange(
    rawTop.map((x) => x.csRaw),
    0,
    10
  );
  const goldRange = statRange(
    rawTop.map((x) => x.goldRaw),
    0,
    500
  );
  const laneRange = statRange(
    rawTop.map((x) => x.laningRaw),
    -500,
    500
  );
  ...
  return {
    ...
    overview: {
      currentRank,
      seasonWinRate: rankedGames
        ? `${round((totalWins / rankedGames) * 100, 1)}%`
        : "0.0%",
      recentGames: rankedGames,
      avgKDA: round(
        safeDivide(totalKills + totalAssists, Math.max(1, totalDeaths)),
        2
      ),
      wins: totalWins,
    ...
    meta: {
      ...
      matchCountUsed: PROFILE.matchCount,
